...
bool isBlue(const Vec3b& pixel) {
	return pixel[0] > 150 && pixel[1] < 100 && pixel[2] < 100;
}

bool checkPepsiLogo(const Mat& image) {
	for (int y = 0; y < image.rows; ++y) {
		for (int x = 0; x < image.cols; ++x) {
...
void generateLabelPepsiLogo(set<myStruct> testSet, set<myStruct>& newSet, const char* message) {
	for (set<myStruct>::iterator i = testSet.begin(); i != testSet.end(); i++) {
		Mat image = imread(i->filePath);
		int newLabel = checkPepsiLogo(image) ? 0 : 1;
...
int main()
{
	...
	do
	{
		...
		printf(" 7 - Compute RGB per image for train set\n");
		printf(" 8 - Generate label using euclidian distance\n");
		printf(" 9 - Compute accuracy by euclidian distance\n");
		printf(" 10 - Put incorrect images in folder for euclidian distance method\n");
		printf(" 11 - Put incorrect images in folder for the method which determines if a photo contains more red or blue\n");
		printf(" 12 - Generate label according to color blue\n");
		printf(" 13 - Compute accuracy by color blue\n");
		printf(" 14 - Put incorrect images in folder for method which computes around color blue \n");
		printf(" 15 - Test BATCH opening\n");
		printf(" 0 - Exit\n\n");
		printf("Option: ");
		scanf("%d", &op);
		switch (op)
		...
		case 10:
			moveIncorrectImages(test, newTestSetRGB, "D:/PI/archive/incorrect_predicted_images");
			printf("Press 'e' to exit.\n");
			while (true) {
				if (_kbhit()) {
					char ch = _getch();
			...
			break;
		case 11:
			moveIncorrectImages(test, newSetByColor, "D:/PI/archive/incorrect_predicted_images_red_blue");
			printf("Press 'e' to exit.\n");
			while (true) {
				if (_kbhit()) {
					char ch = _getch();
			...
			break;
		case 12:
			generateLabelPepsiLogo(test, newSetPepsiLogo, "Generating labels for test images\n");
			printf("Press 'e' to exit.\n");
			while (true) {
				if (_kbhit()) {
					char ch = _getch();
			...
			break;
		case 13:
			computeAccuracyByColor(test, newSetPepsiLogo);
			printf("Press 'e' to exit.\n");
			while (true) {
				if (_kbhit()) {
					char ch = _getch();
			...
			break;
		case 14:
			moveIncorrectImages(test, newSetPepsiLogo, "D:/PI/archive/incorrect_predicted_images_blue");
			printf("Press 'e' to exit.\n");
			while (true) {
				if (_kbhit()) {
					char ch = _getch();
